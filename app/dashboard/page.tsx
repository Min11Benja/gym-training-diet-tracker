...
export default function DashboardPage() {
  // Use local date instead of UTC to avoid off-by-one errors near midnight
  ...
  const caloriesConsumed = logs.reduce(
    (acc: number, item: any) => acc + item.calories,
    0,
  );
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const proteinConsumed = logs.reduce(
    (acc: number, item: any) => acc + item.protein,
    0,
  );
  ...
  const weightData = metrics
    ? [...metrics].reverse().map((m) => ({ w: m.weight }))
    : [];
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <p className="text-zinc-600 dark:text-gray-400 text-sm uppercase tracking-wider font-semibold">
          Today&apos;s Overview
        </p>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Link
          ...
          className="bg-white dark:bg-[#151515] p-5 rounded-2xl border border-zinc-200 dark:border-white/10 relative overflow-hidden group hover:shadow-md dark:hover:shadow-none transition-shadow"
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-zinc-200 dark:bg-zinc-800">
            <div
              className="h-full bg-emerald-600 dark:bg-[#B2FF59] transition-all duration-1000"
              style={{ width: `${calPercent}%` }}
            ></div>
          </div>
          <div className="flex justify-between items-start mb-2">
            <Flame
              className="text-emerald-600 dark:text-[#B2FF59]"
              size={20}
              fill={calPercent > 100 ? "currentColor" : "none"}
            />
            <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400">
              {caloriesGoal - caloriesConsumed} left
            </span>
          </div>
          <div className="mt-2">
        ...
        <Link
          href="/dashboard/nutrition"
          className="bg-white dark:bg-[#151515] p-5 rounded-2xl border border-zinc-200 dark:border-white/10 relative overflow-hidden hover:shadow-md dark:hover:shadow-none transition-shadow"
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-zinc-200 dark:bg-zinc-800">
            <div
              className="h-full bg-emerald-600 dark:bg-[#B2FF59] transition-all duration-1000"
          ...
          <div className="flex justify-between items-start mb-2">
            <span className="text-xs font-bold text-emerald-600 dark:text-[#B2FF59] tracking-wider">
              PROTEIN
            </span>
            <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400">
      ...
      <Link
        ...
        className="block bg-white dark:bg-[#151515] p-5 rounded-2xl border border-zinc-200 dark:border-white/10 flex items-center justify-between hover:shadow-md dark:hover:shadow-none transition-shadow"
      >
        <div>
        ...
        <div
          className={`h-10 w-10 rounded-full flex items-center justify-center ${workoutStatus === "Done" ? "bg-emerald-600 dark:bg-[#B2FF59] text-white dark:text-black" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500"}`}
        >
          {workoutStatus === "Done" ? (
      ...
      {todaysWorkout?.todaysWorkout && (
        <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-950/30 dark:to-emerald-900/10 p-6 rounded-2xl border border-emerald-200 dark:border-emerald-800/50">
          ...
          <div className="space-y-3">
            {todaysWorkout.todaysWorkout.exercises.map(
              (
                exercise: {
                  name: string;
                  sets: number;
                ...
                <div
                  key={idx}
                  className="bg-white/50 dark:bg-black/20 p-4 rounded-xl border border-emerald-100 dark:border-emerald-900/30"
                >
      ...
      <Link
        ...
        className="block bg-white dark:bg-[#151515] p-5 rounded-2xl border border-zinc-200 dark:border-white/10 hover:shadow-md dark:hover:shadow-none transition-shadow"
      >
